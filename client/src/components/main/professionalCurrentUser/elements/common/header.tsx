...
const Header: React.FC<HeaderProps> = ({ HeaderType, display }) => {
    ...
    <HeaderContainer display={display}>
      <Wrapper display={display}>
        <IconContainer display={display}>
          <Icon2Container>
            <BorderContainer display={display}>
              <BorderImageContainer>
                <BorderIcon src={getBorderIcon()} display={display} />
              </BorderImageContainer>
              <TopImageContainer>
                <Icon src={getIcon()} display={display} />
              </TopImageContainer>
            </BorderContainer>
          </Icon2Container>
          <Icon1Container>
        ...
        <TextContainer>
          <Text variant={"normal"} size={display === 'fullScreen' ? "2.2rem" : "1.6rem"} fontWeight={"400"}>
            {getHeaderText()}
          </Text>
        </TextContainer>
      </Wrapper>
...
const HeaderContainer = styled.div<{ display: 'fullScreen' | 'notFullScreen' }>`
  display: flex;
  width: 100%;
  height: 100%;
  align-items: center;
...
const IconContainer = styled.div<{ display: 'fullScreen' | 'notFullScreen' }>`
  display: flex;
  position: relative;
  align-items: center;
  justify-content: center;
  width: ${({ display }) => (display === 'fullScreen' ? '4rem' : '3rem')};
  height: ${({ display }) => (display === 'fullScreen' ? '3.5rem' : '2.5rem')};
`;
...
const Wrapper = styled.div<{ display: 'fullScreen' | 'notFullScreen' }>`
  display: flex;
  width: 100%;
  height: 100%;
  align-items: center;
  justify-content: flex-start;
  gap: ${({ display }) => (display === 'fullScreen' ? '1.5rem' : '1rem')};
`;

const BorderContainer = styled.div<{ display: 'fullScreen' | 'notFullScreen' }>`
  position: relative;
  display: flex;
  align-items: center;
  justify-content: center;
...
const BorderIcon = styled.img<{ display: 'fullScreen' | 'notFullScreen' }>`
  width: ${({ display }) => (display === 'fullScreen' ? '2.2rem' : '1.7rem')};
  height: ${({ display }) => (display === 'fullScreen' ? '2.2rem' : '1.7rem')};
`;

const Icon = styled.img<{ display: 'fullScreen' | 'notFullScreen' }>`
  width: ${({ display }) => (display === 'fullScreen' ? '2rem' : '1.6rem')};
  height: ${({ display }) => (display === 'fullScreen' ? '2rem' : '1.6rem')};
`;
